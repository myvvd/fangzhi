...
class Enroll extends HomeBase
{
    ...
    public function save($data = [])
    {
		...
		if($time>1652248800){ //1652488200 // 1652508000  //1652428800
			$this->error('抱歉，当前日期已过提交作品阶段，无法提交新作品');
		}elseif($time>1652229000){
			//
		}elseif($time>1652169600){
			$this->error('抱歉，当前日期已过提交作品阶段，无法提交新作品');
		}
        ...
        if (($data['id'] !== null) && !empty($data['id'])) {
            //unset($data['uid']);
            $submitto = $model->where('id', $data['id'])->field('submitto,checkstatus1,check1_remark2,check1_remark')->find()->getData();
            ...
            if ($submitto['checkstatus1'] == -1) {
                ...
                $data['checkstatus1'] = 0;
				unset($data['id']);
				$data['check1_remark2'] = $submitto['check1_remark2'];
				$data['check1_remark'] = $submitto['check1_remark'];
    ...
    public function detail()
    {
       ...
       // $result = $result->getData();
		...
		$result['patent'] = $result['patent']!==''? explode(",",$result['patent']):[];
