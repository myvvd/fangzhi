...
////
        ...
        if (cookie('rolecode') == 'specialist') {
            ...
            if ($currentStage == 'chusai') {
                ...
                if ($expertGroupName == '初评1' || $expertGroupName == '初评2' || $expertGroupName == '初评3') {
                    unset($map[2]);
                    $splitId = 1426;
                    if ($expertGroupName == '初评1') {
                        /*$map[] = [['gc_entry_work.id', '<', $splitId]];
                        $map[] = ['gc_entry_work.works_category', '=', 'A'];*/
						$where = "((w.id<=983 and w.declaration_group=41 and w.works_category=29) or (w.works_category IN (".session('workscategory').") and w.works_category<>29))  ";
                    } elseif ($expertGroupName == '初评2') {
						
                        $where = "((w.id>={$splitId} and w.declaration_group=41 and w.works_category=29) or (w.declaration_group=40 and w.works_category=29))";
						
                    } elseif ($expertGroupName == '初评3') {
                        $where =  "((w.id>983 and w.id<{$splitId} and w.declaration_group=41 and w.works_category=29) or (w.works_category IN (".session('workscategory').") and w.works_category<>29))  ";
                    }
            ...
            if (!empty($export)) {
                //专家导出打分表格数据
                $list = $query->where(json_decode($export, true))->where($map)->where($where)->field($fields)->order('workcode desc,w.id desc')->select()->toArray();
            } else {
                $list = $query->where($map)->where($where)->field($fields)->order('g.totalscore desc,w.id desc')->page($page, $limit)->select()->toArray();
            }
	...
	public function edit()
	{
		...
		unset($info['check1_remark2']);
		unset($info['check1_remark']);
    ...
    public function pingfen($wid = '')
    {
		...
		if ($expertGroupName == '初评1' || $expertGroupName == '初评2' || $expertGroupName == '初评3') {
			...
			if($map_len>0) unset($map[$map_len-1]);
			$splitId = 1426;
			if ($expertGroupName == '初评1') {
				$where = "((id<=983 and declaration_group=41 and works_category=29) or (works_category IN (".session('workscategory').") and works_category<>29))  ";
			} elseif ($expertGroupName == '初评2') {
				
				$where = "((id>={$splitId} and declaration_group=41 and works_category=29) or (declaration_group=40 and works_category=29))";
				
			} elseif ($expertGroupName == '初评3') {
				$where =  "((id>983 and id<{$splitId} and declaration_group=41 and works_category=29) or (works_category IN (".session('workscategory').") and works_category<>29))  ";
			}
		}
		
		
        $nextWorksId = Db::name('entry_work')->where('checkstatus1', '=', 1)->where($map)->where('id', '>', $wid)->where($where)->order('id asc')->limit(1)->value('id');
    ...
    public function saveGrade($data = [])
    {
      ...
      case 'final':
        ...
        $nextWorksId = Db::name('entry_work')->where('checkstatus1', '=', 1)->where($map)->where('id', '<', $data['work_id'])->where($where)->order('id desc')->limit(1)->value('id');
        if ($nextWorksId) {
    ...
    public function pingfendetail($wid = '')
    {
        ...
        $expertType = cookie('experttype');
        if ($expertType == 'chusai') {
            $pingfenList = Db::name('grade')->field('g.id as gid,g.*,a.*')
        ->alias('g')
        ->where('work_id', '=', $wid)
        ->join('admin a', 'g.adminid=a.id')
        ->select();
        } else {
            $pingfenList = Db::name('grade_'.$expertType)->field('g.id as gid,g.*,a.*')
        ->alias('g')
        ->where('work_id', '=', $wid)
        ->join('admin a', 'g.adminid=a.id')
        ->select();
	...
	public function updatepingfen($id = 0,$score=0,$wid=0)
    {
        if (empty($id)) {
            $this->error('参数错误');
        }
